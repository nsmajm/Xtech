...
@section('content')
    ...
    <div class="card">
        <div class="card-body">
            <p class="float-right">
                <button class="btn btn-success float-right" data-toggle="modal" data-target="#specification" id="addNew"
                        data-whatever="@getbootstrap">Add New Specification
                </button>
            </p>
            <div class="container mt-5" >
                <form  method="post" action="{{route('product.updateNext')}}">
                    ...
                    <div id="specificationList" class="mt-4 mb-4">
                        {{--<div class="row">--}}
                            ...
                            {{--</div>--}}
                            {{--<div class="col-md-6">--}}
                                {{--<textarea name="SpecificationValue[]" id="SpecificationValue" class="form-control" required></textarea>--}}
                            {{--</div>--}}
                            {{--<div class="col-md-2">--}}
                        ...
                        @foreach($specId as $item)
                            <div class="row mt-4">
                                {{--<input type="hidden" value="{{$item->id}}" name="deleteList[]">--}}
                                <div class="col-md-4 mt-3">
                                    <select name="specificationId[]"  id="specificationId" class="form-control">
                                        @foreach($productSpec as $key=>$value)
                                        <option value="{{$value->name}}" {{ $value->name == $item->spec_name ? 'selected="selected"' : '' }}>{{$value->name}}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="col-md-6">
                                    <textarea name="SpecificationValue[]" id="SpecificationValue" class="form-control">{{$item->spec_value}}</textarea>
                                </div>
                                <div class="col-md-2">
                                    <button type="button" class="btn btn-danger" id="deleteItem"
                                            data-panel-id="{{$item->id}}" onclick="deleteSingleSpecification(this)">
...
@section('js')
    ...
    <script>
        ...
        $('#addNew').click(function () {
            ...
            function deleteSingleSpecification(x) {
                $.confirm({
                    title: 'Confirm!',
                    ...
                    buttons: {
                        confirm: function () {
                            ...
                            $.ajax({
                                type: 'POST',
                                url: "{!! route('product.single.spec.delete') !!}",
                                cache: false,
                                data: {_token: "{{csrf_token()}}",'id': id},
                                success: function (data) {
                                    $('#specificationList').load(location.href +' #specificationList');
